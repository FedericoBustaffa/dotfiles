...
--
-- local function viewer_alive()
--   if not state.zathura_pid then
--     return false
--   end
--   local res = vim.fn.system('kill -0 ' .. state.zathura_pid .. ' 2>/dev/null; echo $?')
--   return vim.trim(res) == '0'
-- end
--
-- -- Chiude zathura in modo asincrono, chiama callback quando è morto
-- local function close_viewer(callback)
--   if not viewer_alive() then
--     state.zathura_pid = nil
--     if callback then
--       callback()
--     end
--     return
...
--     attempts = attempts + 1
--     local res = vim.fn.system('kill -0 ' .. pid .. ' 2>/dev/null; echo $?')
--     if vim.trim(res) == '0' then
--       if attempts >= 8 then
...
--   })
--
--   state.zathura_pid = vim.fn.jobpid(job_id)
--
--   if is_hyprland() then
--     move_to_workspace_8(state.zathura_pid)
--   end
-- end
--
-- -- ── Watch ────────────────────────────────────────────────────────────────────
...
--   vim.notify('Typst: compiling ' .. vim.fn.fnamemodify(file, ':t') .. '...', vim.log.levels.INFO)
--
--   -- Chiudi viewer corrente → compila → apri nuovo
